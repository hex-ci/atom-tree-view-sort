...
let sortEntries = function(combinedEntries) {
  combinedEntries.forEach((name, key) => {
    if (typeof name === 'string') {
      ...
      ['atime', 'birthtime', 'ctime', 'mtime'].forEach((value) => {
        let time = statFlat[value];
        statFlat[value] = time ? time.getTime() : 0;
      });
  ...
  let result = combinedEntries.sort((first, second) => {
...
export default {
  ...
  _initialize() {
    ...
    this.subscriptions.add(atom.config.onDidChange('tree-view-sort.type', ({newValue, oldValue}) => {
      sortType = newValue;
    ...
    this.subscriptions.add(atom.config.onDidChange('tree-view-sort.descending', ({newValue, oldValue}) => {
    ...
    this.subscriptions.add(atom.config.onDidChange('tree-view-sort.caseSensitive', ({newValue, oldValue}) => {
    ...
    this.subscriptions.add(atom.config.onDidChange('tree-view-sort.sortFoldersBeforeFiles', ({newValue, oldValue}) => {
