...
const sort = (first, second, firstName, secondName, isSortByName) => {
  const descending = (isDescending && !isSortByName) ? -1 : 1;
  const curSortType = isSortByName ? 1 : sortType;
    ...
    if (isDir(first) && sortFolders != 0) {
      if (isDir(second)) {
        if (firstName.indexOf('.') === 0 && secondName.indexOf('.') !== 0) {
          ...
          return sort(first, second, firstName, secondName, (sortFolders==3)?1:0);
        }
      }
      else {
        return (sortFolders == 1 || sortFolders == 3) ? -1 : 1;
      }
    }
    else {
      if (isDir(second) && sortFolders != 0) {
        ...
        if (firstName.indexOf('.') === 0 && secondName.indexOf('.') !== 0) {
          ...
          return sort(first, second, firstName, secondName, 0);
        }
      }
    }
  });
