...
const getFileGroup = (filename) => {
  const count = (filename.match(/\./g) || []).length;
  const arr = filename.split('.');
  return count < 2 ? '' : arr[arr.length-2];
};
...
const sort = (first, second, firstName, secondName) => {
  ...
  switch (sortType) {
    ...
    case 3: {
      const firstGroupName = getGroupExtension(firstName);
      const secondGroupName = getGroupExtension(secondName);
